...
A bounded sweep of recent stored replies and a fresh Claude Opus review found repeated red raw commands when hand-authored colour and underline groups ended together. In one reply, expressions with ordinary text after a nested group rendered while an expression ending immediately after that group lacked an outer closing brace. The checker caught the malformed expression when run later, but requiring it before every message did not address agents omitting that step. Keep ordinary prose in Markdown, use short complete underline or colour expressions without hand-nesting them, and use a colour switch inside `\textsf` so the hand-written forms end consistently with `}\)`. Generated rainbow quotes are a distinct fixed exception. The optional checker remains useful for diagnostics and repository tests; do not turn it into a per-message dependency. User-requested simplification — 2026-09-22. The new patterns passed KaTeX rendering and dedicated reply tests.
...
## Keep underlines separate from sans-serif styling

The live Compare Zap and Deliveroo task had read the installed skill but still emitted red raw `\textsf{\underline{...` in three commentary updates. Each example closed the inner underline and math delimiter but omitted the closing brace for `\textsf`. Earlier copyable examples and optional diagnostics did not prevent that authoring mistake. Ethan explicitly wants the underlined scanning cues kept. Use the complete short `\(\underline{\text{clue with spaces}}\)` expression in working and final replies, without an outer `\textsf`; keep colour and About in separate short sans-serif `\(\textsf{\color{#hex}...}\)` expressions. Ordinary Markdown prose already uses the app’s sans-serif font. The `\text` wrapper preserves spaces: KaTeX renders bare `\underline{words with spaces}` without the spaces. This supersedes the earlier advice to drop underlines or avoid `\textsf` everywhere. Keep the font command only in non-underlined styling. Keep the quote generator and skill-name announcement as their own controlled exceptions. The generated rainbow question uses a checked outer `	extsf` without underlining, retaining the sans-serif preference and avoiding the hand-authored nested-underline error. Do not add an automatic repair turn or per-message checker. User correction, clarification and KaTeX rendering check — 2026-09-23.
