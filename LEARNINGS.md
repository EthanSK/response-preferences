...
## Valid combined syntax, simpler underline authoring

In the live **Compare Zap and Deliveroo** task, all 13 assistant-authored expressions with ordinary words after the inner underline had balanced braces; all three expressions where that underline was the final content omitted the outer `\textsf` closing brace. The repository already had tests proving properly closed combined forms render. A direct KaTeX check also rendered both `\underline{\textsf{still pending}}` and `\textsf{\underline{still pending}}` with the sans-serif class and spaces intact, while either form without its final brace failed. The combination is valid, so do not claim otherwise. Ethan then clarified that underlined text still does not give him the whole-line triple-click selection he wanted. Its extra font nesting therefore has no useful payoff for underline cues. Use the complete short `\(\underline{\text{clue with spaces}}\)` as the authoring recipe, keep ordinary prose outside the math box, and leave selected colour/About statements in their own short sans-serif expressions. The optional diagnostic remains optional; do not add an automatic repair turn or per-message checker. User correction and verified rendering — 2026-09-23.
