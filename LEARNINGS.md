...
## Keep underlines without the extra font wrapper

The live Compare Zap and Deliveroo task had read the installed skill but still emitted red raw `\textsf{\underline{...` in three commentary updates. Each example closed the inner underline and math delimiter but omitted the closing brace for `\textsf`. Earlier copyable examples and optional diagnostics did not prevent that authoring mistake. Ethan explicitly wants the underlined scanning cues kept. Use the complete short `\(\underline{\text{clue with spaces}}\)` expression in working and final replies, without an outer `\textsf`; keep any colour in a separate short `\(\color{#hex}\text{...}\)` expression. The `\text` wrapper preserves spaces: KaTeX renders bare `\underline{words with spaces}` without the spaces. This supersedes the earlier advice above to use `\textsf` or to drop underlines for Markdown bold. Keep the quote generator and skill-name announcement as their own controlled exceptions. Do not add an automatic repair turn or per-message checker. User correction and KaTeX rendering check — 2026-09-23.
