...
4. Every outgoing reply needs its own check; a previous reply or repository-test pass is not evidence for the current draft. Save final Markdown and run `scripts/check-reply.py /absolute/reply.md`. Fix failures, then prepare notification metadata separately using the installed native integration, if present. If the draft changes afterwards, check and prepare again.
...
The draft checker requires formatted paragraph prose to stay inside consecutive short text expressions, rejects prose left outside them, and rejects selectable chunks above 64 approximate visible characters. It does not measure glyph widths. Links, code, paths, literal quotes and mathematical expressions without prose text commands remain explicit exceptions.
