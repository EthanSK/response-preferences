...
def operate(db_path, identity, action, token=None, outcome=None, revision=None, now=None):
    ...
    try:
        ...
        if action == 'claim':
            ...
            elif state.get('next_check_at', 0) > now:
            ...
            if action == 'renew':
                ...
                if outcome not in ('unchanged', 'updated', 'blocked', 'error'):
                    raise ValueError('Choose unchanged, updated, blocked, or error.')
                if outcome in ('unchanged', 'updated') and not revision:
                    raise ValueError('A verified upstream revision is required.')
                state.update(last_attempt_at=now, outcome=outcome,
                             next_check_at=now + (WEEK if outcome in ('updated', 'unchanged') else 86400 if outcome == 'blocked' else 3600))
...
def main():
    ...
    parser.add_argument('--outcome', choices=['unchanged', 'updated', 'blocked', 'error'])
    ...
    try:
        print(json.dumps(operate(base / 'agent-skill-updates' / 'state.sqlite3', identity, args.action,
                                 args.token, args.outcome, args.revision)))
