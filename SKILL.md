...
- With that guard installed, read its exact-format exception procedure when the user explicitly requests undecorated output. Bind the exception to that task and exact reply; never use it merely to avoid styling. A complaint about broken formatting asks for repair, not plain text: verify the quoted request explicitly requires a conflicting output format before granting an exception. Self-improved — 2026-09-09: an earlier formatting complaint was incorrectly treated as permission to drop the style; see the negative example in [verification guidance](references/verification.md#formatting-complaints-are-not-exceptions).
...
A coloured span must make sense from its first word without surrounding text. Keep colour selective inside the paragraph's shared outer `\textsf` wrapper. Codex treats separate KaTeX expressions as selection boundaries, while one user-tested outer expression lets native selection cover the whole paragraph. Keep each selectable paragraph concise: the checker rejects an outer prose expression above 120 approximate visible characters, which reduces but cannot eliminate overflow risk. Split longer writing into new paragraphs rather than new expressions on the same paragraph line. Preserve essential negatives and conditions; do not shrink fonts or force desktop-specific line breaks.
...
**Wrap each assistant-authored prose paragraph in one normal-size `\(\textsf{...}\)` expression**, then nest its approved colours and `\underline{...}` cues inside that same outer wrapper. Keep the section marker outside. This makes the paragraph one selectable KaTeX element instead of several selection islands. Use a new short paragraph when the outer expression would exceed 120 approximate visible characters. Links, inline code, paths, literal quotations and other content KaTeX cannot safely contain remain outside as narrow exceptions; group the surrounding prose as far as the safe boundary allows. Headings, labels, tables and code blocks are not prose paragraphs. User correction — 2026-09-18.

**Underline useful clues in every assistant-authored prose sentence**, in working and final replies. Inside the paragraph wrapper, use `\underline{useful words}`; the outer `\textsf` already provides text mode. The first cue in each paragraph/section must establish its concrete subject; later cues can give method, result, quantity or qualification. Read the cues alone in order and as first-plus-later pairs. Preserve `not`, `only`, conditions and uncertainty. Avoid detached opening cues like “agent-based” without saying what is agent-based. Do not underline whole sentences mechanically or filler words.
...
End **only the final reply** with one lavender `#b8a4d9` **About:** reminder. Do not add About reminders to working commentary or progress updates. Put its two short sentences inside one normal-size lavender `\textsf` expression so the reminder is selectable as one paragraph. Use a concrete overall subject, then useful explanation/action/result/next step. Keep the combined expression within the 120-character guardrail. No underline, emoji, caret, link, heading or divider in this reminder. It comes after any applicable outstanding-item or environment footer and does not replace the question quote.
