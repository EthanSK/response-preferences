...
Style that short question reminder as **rainbow text**, preserving its words. Use separate normal-size `\(\color{#f87171}{\textsf{Your words}}\)` chunks with ordinary spaces, no underline, cycling `#f87171`, `#fb923c`, `#facc15`, `#4ade80`, `#22d3ee`, `#60a5fa`, `#c084fc`. Use 1–3 words and at most 24 visible characters per chunk; restart the colours for each quote. This palette is only for quoted question reminders, not status highlights or the full annotation/evidence block. Use [the fixed recipe and variable-length examples](references/format-recipes.md#rainbow-question-reminders), or run `python3 <skill-dir>/scripts/rainbow-quote.py <question.txt>` to get escaped, renderer-checked output without rebuilding the syntax. The same reference has copyable recipes for outcomes, tests, pointers and About reminders.
