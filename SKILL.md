...
- Keep underlines in working and final replies. Use the complete short `\(\underline{\text{clue with spaces}}\)` pattern below; do not nest `\textsf` in an underline or wrap an underline with it. The font wrapper did not give Ethan the triple-click grouping he wanted, so keep underline cues simple. Keep `\textsf` for separate colour, About and rainbow expressions. User correction — 2026-09-23.
...
Use the complete short underline expression below for scanning. `\text{...}` keeps the words and spaces together; `\underline{...}` draws the line. It ends `}}\)`: one brace closes `\text`, one closes `\underline`. Do not add `\textsf` to this expression. Separate colour and About patterns retain their sans-serif styling. Keep each expression short enough to wrap around ordinary Markdown. User correction — 2026-09-23.
...
**Write prose as ordinary Markdown around short expressions.** Use `\(\underline{\text{short useful words}}\)` for scanning and the separate `\(\textsf{\color{#67e8f9}A complete short fact.}\)` when colour helps. A coloured sentence already carries the scanning message; do not repeat it solely to add an underline. Keep expressions short enough to fit, with ordinary spaces around them for wrapping. User correction — 2026-09-23.
