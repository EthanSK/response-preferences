...
- Immediately above each direct-answer ⮑ line, put one short, simple phrase summarising what Ethan asked, followed by `[↗](absolute-context-viewer.html)`. Use a Markdown hard line break so the arrow starts on the next line and looks like it comes out of that reminder. Keep the reminder unmarked; the answer's approved arrow remains first on its own line.
