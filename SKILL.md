...
For ordinary original-message hover, the deliberate fake path `[↗](</Exact original user text>)` is a hover-only exception, not a working file link. Do not invent native annotation indices or promise a custom hover UI. For real attached annotations, use the required `:codex-annotation{index="N"}` and its native popup instead of duplicating it with a fake path. Detailed annotation replies retain the full Problem at hand / Earlier response / Your annotation quote, then a separate short relevant-question quote before ⮑; group related annotations without losing any original context. These rules work in tandem: generic response styling, the rainbow question and the native annotation popup never override or replace that full context, including when answering a later request to clarify earlier annotations. Before sending, check every addressed annotation has all three nonempty labelled fields; the draft checker rejects native annotation references without them. Where Ethan’s completion adapter is installed, it also checks annotation context independently of the optional broad-style switch; updating the checker alone does not prove cross-chat enforcement. Verify the installed adapter with that switch off. User correction — 2026-09-20.
