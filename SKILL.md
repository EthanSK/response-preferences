...
- Put each approved section marker on its own line, left-aligned, immediately above its section’s title or opening text, with a blank line between the marker and content. Do not place the title or prose beside the marker, append the marker to a heading, centre it, or indent it. The marker applies to all content below it until the next section marker.
- Render section markers using inline LaTeX with lowercase `\huge`, for example `\(\huge\text{✅}\)`, alone on their line. Enlarge only the marker; keep prose in ordinary Markdown and approved coloured highlights in separate normal-size LaTeX spans below it. A trial or question about other sizes does not change this default without Ethan’s explicit selection.
- Start a new marked section when the content’s purpose changes. A ✅ result does not cover subsequent explanations, limitations or decisions: use ⓘ for supporting information, ⚠️ for cautions, 🫵 for Ethan’s decisions, or another matching approved marker. Group consecutive content with the same purpose under one marker; do not repeat it for every paragraph. Every ordinary content section should have a meaningful marker, but never invent one if the vocabulary does not fit. Original-message blockquotes and annotation quotations are context for the following answer, not extra information sections; table-cell markers remain inline as specified below.
...
- Immediately above each direct-answer ⮑ line, put only the question or specific part of Ethan’s message that the answer underneath addresses, followed by `[↗](absolute-context-viewer.html)`. Put the reminder and its opening link in a Markdown blockquote (`> reminder [↗](absolute-context-viewer.html)`) so it has a vertical line on the left. Leave a blank line after the blockquote, then put the direct-answer arrow alone on its own line outside it, followed by a blank line and the answer. Do not add a section emoji to the reminder; the answer's approved arrow remains first on its own line.
...
- Put the section marker on its own line above any title or coloured highlight. For a successful result, use `\(\huge\text{✅}\)`, a blank line, then `\(\color{#22c55e}{\textsf{All checks passed.}}\)`. Start a new ⓘ section when the content changes to supporting information.
