...
- Before a final reply, save the exact draft and run `python3 <skill-dir>/scripts/check-reply.py <reply.md>`. Fix errors before notification preparation. Use `--commentary` when checking work updates. A passing checker proves only its mechanical checks, not correct meaning or visual rendering. If unavailable, check manually and state that limitation.
