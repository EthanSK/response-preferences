...
| Sentence scanning | Prefers a short useful underline in prose sentences. A self-contained coloured sentence already serves as a scanning cue. Keeps negatives and conditions so scanning does not change the meaning. Exact quotes, code and links remain intact. |
...
Codex treats each inline LaTeX expression as an unbreakable box. Ethan verified continuous selection in one coloured expression, then showed that whole-paragraph boxes still overflow. A later underline-and-emoji trial did not give whole-line triple-click selection. Write ordinary prose as Markdown, and style only short cues or self-contained highlights. Every hand-written expression should end with a normal letter or punctuation mark, then exactly `}\)`; close any underline before that last character. This removes the variable two- and three-brace endings that repeatedly broke. Keep each expression below 64 approximate visible characters. The generated rainbow question is a separate helper-produced exception.

In working updates and finals, keep literal names such as `C#` and `sample_tool` in ordinary text or inline code, with short readable scanning cues nearby. Write percentages outside the LaTeX box too. If they must be styled, escape the text as `C\#`, `sample\_tool`, `54\%` or `A \& B`. This applies to coloured text, underlines and About reminders.

Start with the [copyable LaTeX patterns](SKILL.md#copy-these-exact-latex-patterns): a cue uses `\(\textsf{\underline{short clue} plus context}\)`, colour uses `\(\textsf{\color{#67e8f9}A complete short fact.}\)`, and About uses the same switch form in lavender. The checker remains an optional diagnostic for tricky expressions or skill maintenance. It validates syntax with bundled KaTeX but cannot guarantee agent obedience or the app’s layout. Annotation replies still require their labelled context, rainbow question and native reference. Automatic reply repair is not used.
...
Prefer a short underlined clue in assistant prose sentences: the subject, action, result or qualification that gives away their meaning at a glance. A self-contained coloured sentence already serves as a scanning cue. Make the first underline in a paragraph name its concrete subject. Keep crucial negatives or limits, such as **not uploaded** or **after restarting**. Avoid filler and whole-sentence underlines. Exact quotations, code, paths and link labels stay intact.

Write the surrounding sentence as ordinary Markdown. For example: `The backup is \(\textsf{\underline{not uploaded}.}\)` or `The \(\textsf{\underline{review needs a} project}\) written in` `C#`. Close the underline before the last ordinary character inside the expression, so it ends in `}\)`. Colour highlights use a separate short expression; do not hand-nest colour and underline. The website uses styled `<u>` spans internally; never paste those HTML tags into an assistant reply.
...
The everyday pattern is deliberately small: write normal Markdown, insert a short `\(\textsf{\underline{useful} clue}\)` when an underline helps, and use `\(\textsf{\color{#67e8f9}A complete short fact.}\)` for a selected colour highlight. Each hand-written expression ends in a normal letter or punctuation mark and one closing brace. Leave surrounding prose, links, code and technical names in Markdown so they wrap naturally. Avoid hand-nesting colours and underlines. The final `About:` reminder and generated rainbow question retain their own fixed patterns.
...
Use short selectable lavender chunks when needed: `\(\textsf{\color{#b8a4d9}About: fixing video export.}\) \(\textsf{\color{#b8a4d9}Restart the app, then retry the clip.}\)`. Keep each chunk within the 64-character guardrail and separate chunks with ordinary spaces. Muted lavender is reserved for this one closing reminder, with one `About:` prefix and no extra emoji, underline or divider. The relevant first-person quote remains above its answer; the reminder stays last after any outstanding-item recommendation or applicable environment details.
