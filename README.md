...
Use short selectable lavender chunks when needed: `\(\color{#b8a4d9}\text{About: fixing video export.}\) \(\color{#b8a4d9}\text{Restart the app, then retry the clip.}\)`. Keep each chunk within the 64-character guardrail and separate chunks with ordinary spaces. Muted lavender is reserved for this one closing reminder, with one `About:` prefix and no extra emoji, bold or divider. The relevant first-person quote remains above its answer; the reminder stays last after any outstanding-item recommendation or applicable environment details.
