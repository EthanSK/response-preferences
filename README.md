...
Codex treats each inline LaTeX expression as an unbreakable box. Ethan verified continuous selection in one coloured expression, then showed that whole-paragraph boxes still overflow. A later test containing both an underline and an emoji did not give him whole-line triple-click selection, so the skill does not promise that behavior. Split prose into consecutive `\textsf` chunks of at most 64 approximate visible characters, nesting colours and underlines inside each and leaving ordinary spaces between chunks so the browser can wrap. This is a guardrail, not a guarantee at every pane width or zoom. Links, code, paths and literal quotations remain outside when KaTeX cannot safely contain them.

In working updates and finals, keep literal names such as `C#` and `sample_tool` in ordinary text or inline code, with short readable scanning cues nearby. Write percentages outside the LaTeX box too. If they must be styled, escape the text as `C\#`, `sample\_tool`, `54\%` or `A \& B`. This applies to coloured text, underlines and About reminders. Check the exact working draft with `--commentary`, and check the exact final before sending. Each reply needs a fresh successful check of its complete draft; send the checked text unchanged, and recheck after any edit. Earlier reply checks and repository tests do not cover a new message.
...
The checker now validates explicit math expressions with bundled KaTeX, including mismatched delimiters, response-formatting commands outside delimiters and unsupported commands. It needs Node.js on PATH, or an executable selected with `RESPONSE_PREFERENCES_NODE`; no npm install or network is needed for a copied skill. If the renderer cannot run, the check fails rather than reporting success. Use ordinary Markdown for prose if validation is unavailable. Annotation replies require the full labelled context block as well as the separate rainbow question and native reference, including later clarifications. Check the exact draft before sending. Do not add automatic reply-repair hooks or continuation turns; existing completion notifications remain notification-only. Parsing cannot guarantee layout or model obedience.
...
Use short selectable paragraph chunks: `\(\textsf{The \underline{backup} was \underline{not uploaded}.}\) \(\textsf{The provider was unavailable.}\)`. For colour and underlines together: `\(\textsf{\color{#67e8f9}{\underline{The viewer} is a snapshot.}}\) \(\textsf{It mirrors the file.}\)`. Put the marker outside, nest colour and scanning cues inside each chunk, and split the paragraph at the 64-character guardrail with ordinary spaces between expressions. Underlining is not another importance level and does not make text clickable. The website uses styled `<u>` spans internally; never paste those HTML tags into an assistant reply, where the Codex client can display them literally.
...
Automatic reply repair is not part of this workflow. Harden and follow the rules, then run the explicit draft check before sending. Do not connect legacy guard helpers to a completion hook or wake another task to repair formatting.
...
Use short selectable lavender chunks when needed: `\(\color{#b8a4d9}{\textsf{About: fixing video export.}}\) \(\color{#b8a4d9}{\textsf{Restart the app, then retry the clip.}}\)`. Keep each chunk within the 64-character guardrail and separate chunks with ordinary spaces. Muted lavender is reserved for this one closing reminder, with one `About:` prefix and no extra emoji, underline or divider. The relevant first-person quote remains above its answer; the reminder stays last after any outstanding-item recommendation or applicable environment details.
