...
| Closing topic reminder | Every assistant message ends with a muted-lavender (`#b8a4d9`) `About:` reminder: a brief overall summary, then a second short sentence explaining the specific action, problem or next step for returning readers. It comes after actions and other closing details; the relevant question above each answer stays in place. |
...
In both working updates and finals, prefer percentages in ordinary Markdown, with readable words underlined nearby: underline “Sample polling cost”, then write “was 1–2% of one core” outside the LaTeX. Check LaTeX working drafts with `--commentary`; the automatic completion guard only handles finals. If a percentage must appear inside a LaTeX highlight or underline, escape it: `\(\underline{\textsf{Coverage is 54\%}}\)` renders “Coverage is 54%”. A bare `%` starts a TeX comment and can make the expression appear as red raw syntax. Ordinary Markdown percentages stay unchanged. Keep identifiers such as `sample_tool` in inline code and underline readable prose around them; inside LaTeX text, use `sample\_tool`. Bare underscores in text wrappers can also expose red raw syntax. The reply checker catches both cases in working and final replies while preserving mathematical subscripts.
