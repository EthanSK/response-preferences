...
Use `The \(\underline{\textsf{backup}}\) was \(\underline{\textsf{not uploaded}}\) because the provider was unavailable.` For colour and underlines together: `\(\color{#67e8f9}{\textsf{\underline{The viewer} is a \underline{snapshot} of the file.}}\)`. Keep every continuous underline to five words or fewer. Split longer cues into separate math expressions with ordinary spaces or prose between them so the line can wrap; splitting underline commands inside one large math expression does not create a wrap point. The whole coloured clause still makes sense independently, while its underlined words provide scanning clues. Underlining is not another importance level and does not make text clickable. The website uses styled `<u>` spans internally; never paste those HTML tags into an assistant reply, where the Codex client can display them literally.
